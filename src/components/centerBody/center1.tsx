...
const Center: React.FC<CenterProps> = ({ items }) => {
  ...
  const getUserAvatar = async (email) => {
    const res = await getDoc(doc(database, "user", email));
    return res.data().photoURL;
  };

  const getCommentUserAvatar = async (email) => {
    const userAvatar = await getUserAvatar(email);
    return userAvatar || "default_avatar_url"; // Provide a default URL if no avatar found
  };
  //!SECTION
  return (
      ...
      {isLoading ? (
        ...
        <Flex direction="column">
          ...
          {firebaseData &&
            firebaseData?.map((item, index) => (
              <Flex key={index} direction="column">
                {/* {console.log("items from ssr", getImg(item))} */}
                    ...
                    item.comment
                      ...
                      .map((comment, commentIndex) => (
                        ...
                        >
                          ...
                          >
                            <Image
                              src={getCommentUserAvatar(comment.email)}
                              boxSize="25px"
                              borderRadius="40%"
                            />
                          </Flex>
